...
HINT: bice vam lakse da radite ulancano trazenje. findElement().findELement()...
		...
		/*
		List<WebElement> tabela =
				driver.findElements(By.xpath("//*[@id='lorem']//tbody//td")); */
		//II nacin: ulancano trazenje
		
		List<WebElement> tabela = 
				driver.findElement(By.id("lorem")).findElements(By.tagName("td"));
		
		for (int i = 0; i < tabela.size(); i = i + 3) {
			System.out.println(tabela.get(i).getText() + "\t" + tabela.get(i + 1).getText() + "\t" + tabela.get(i + 2).getText());
		}
		Thread.sleep(5000);
		driver.quit();
